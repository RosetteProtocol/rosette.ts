...
function sameFraction(first: string, second: string): boolean {
...
export const tenPow = (x: any): BigNumber =>
  BigNumber.from(10).pow(BigNumber.from(x));
...
export const formatBN = (
  ...
  const fractionAfterPrecision = (
    fixed ? fraction : fractionWithoutTrailingZeros
  ).slice(0, precision);
